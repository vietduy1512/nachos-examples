...
SynchConsole::SynchConsole()
{
	...
	// 0 cho console input, 1 cho console output, nen index + 2 de chua ra 0 voi 1
	index += 2;
}
...
OpenFile* SynchConsole::GetFile(int _index)
{
	return fileList[_index];
}

int SynchConsole::GetFileType(int _index)
{
	return fileTypeList[_index];
}

int SynchConsole::GetCurrentIndex()
{
	return index;
}

void SynchConsole::SetFile(int _index, OpenFile* file, int type)
{
	fileList[_index] = file;
	fileTypeList[_index] = type;
}

void SynchConsole::IncreaseIndex()
{
	index++;
}
